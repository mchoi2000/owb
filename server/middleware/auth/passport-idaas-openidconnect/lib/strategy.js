...
function Strategy(options, verify) {
  ...
  if (!options.authorizationURL) {
    throw new Error('OpenIDConnectStrategy requires a authorizationURL option');
  }
  if (!options.tokenURL) {
    throw new Error('OpenIDConnectStrategy requires a tokenURL option');
  }
  if (!options.clientID) {
    throw new Error('OpenIDConnectStrategy requires a clientID option');
  }
  if (!options.clientSecret) {
    throw new Error('OpenIDConnectStrategy requires a clientSecret option');
  }
  ...
  this._skipUserProfile = (options.skipUserProfile === undefined) ?
    false : options.skipUserProfile;
}
 ...
 * @return boolean
 */
Strategy.prototype.verifyIssuer = function verifyIssuer(jwtClaims) {
 ...
 * @return boolean
 */
Strategy.prototype.verifyAzp = function verifyAzp(jwtClaims) {
...
Strategy.prototype.authenticate = function authenticate(req, options) {
  ...
  if (req.query && req.query.code) {
    ...
    oauth2.getOAuthAccessToken(code, {
    ...
    }, function getOAuthAccessTokenCallback(err, accessToken, refreshToken, params) {
      if (err) {
      ...
      var idTokenSegments = idToken.split('.'),
        jwtClaimsStr, jwtClaims, idHeader;
      ...
      if (idHeader.alg) {
        if (idHeader.alg.match(/HS(256|384|512)/i)) {
          if (!Array.isArray(jwtClaims.aud) ||
            ...
            jwtClaims.aud.length === 1)) {

            if (!jwtClaims.azp ||
              (jwtClaims.azp && jwtClaims.azp === jwtClaims.aud) ||
              (jwtClaims.azp && jwtClaims === jwtClaims.aud[0])) {
      ...
      if (!(self.verifyIssuer(jwtClaims))) {
        return self.error(
          new Error('Claims for issuer identifier between client and ID Token do not match.'));
      }
      ...
      if (Array.isArray(jwtClaims.aud)) {
        ...
        if (audLength === 0) {
          ...
          if (self._clientID !== jwtClaims.aud[0]) {
            return self.error(
              new Error('Claims for Audience between client and ID Token do not match.'));
          }
        }
      ...
      if (jwtClaims.at_hash) {
        if (idHeader.alg) { // IT SHOULD HAVE IT. ELSE WE GOT A PROBLEM.
          ...
          if (idHeader.alg.indexOf('256') > -1) {
            hash = crypto.createHash('sha256').update(accessToken).digest('hex');
          } else if (idHeader.alg.indexOf('384') > -1) {
            hash = crypto.createHash('sha384').update(accessToken).digest('hex');
          } else {
            hash = crypto.createHash('sha512').update(accessToken).digest('hex');
          }
          ...
          self.error(
            new Error('Access Token can not be verified. AT_Hash claim exists,' +
              ' but algorithm from Header is missing.'));
        }
      }

      self._shouldLoadUserProfile(iss, sub, function _shouldLoadUserProfileCallback(err, load) {
        if (err) {
        ...
        if (load) {
          ...
          }, null, null, function oauthRequestCallback(err, body, res) {
            if (err) {
              return self.error(new InternalOAuthError('failed to fetch user profile',
                err, self.authenticate));
            }
        ...
        function onProfileLoaded(profile) {
          ...
          if (self._passReqToCallback) {
            if (arity === 9) {
              self._verify(req, iss, sub, profile, jwtClaims,
            ...
            if (arity === 8) {
              self._verify(iss, sub, profile, jwtClaims,
                accessToken, refreshToken, params, verified);
            } else if (arity === 7) {
...
Strategy.prototype._shouldLoadUserProfile = function shouldLoadUserProfile(issuer, subject, done) {
  if (typeof this._skipUserProfile === 'function' && this._skipUserProfile.length > 1) {
    // async
    this._skipUserProfile(issuer, subject, function skipUserProfileCallback(err, skip) {
      if (err) {
        return done(err);
      }
      if (!skip) {
    ...
    var skip = (typeof this._skipUserProfile === 'function') ?
      this._skipUserProfile(issuer, subject) : this._skipUserProfile;
    if (!skip) {
      return done(null, true);
    }
    return done(null, false);
